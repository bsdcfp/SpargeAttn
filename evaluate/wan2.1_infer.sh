...
if [ -f "$config_path" ]; then  
  py_file=$(jq -r '.py_file' "$config_path")  
  ckpt_dir=$(jq -r '.ckpt_dir' "$config_path")  
  l1=$(jq -r '.l1' "$config_path")  
  pv_l1=$(jq -r '.pv_l1' "$config_path")  
  model_out_path=$(jq -r '.model_out_path' "$config_path")  
  task=$(jq -r '.task' "$config_path")  
  size=$(jq -r '.size' "$config_path")  
  sample_steps=$(jq -r '.sample_steps' "$config_path")  
  image=$(jq -r '.image' "$config_path")  
  prompt=$(jq -r '.prompt' "$config_path")  
  base_seed=$(jq -r '.base_seed' "$config_path")  
  offload=$(jq -r '.offload' "$config_path")   
else  
